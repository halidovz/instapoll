...
Meteor.methods({
   createPoll: function (name) {
    ...
    var poll =  Polls.insert({name: name});
  ...
  addQuestion: function(poll) {
    Polls.update({privateId: poll}, {$push: {questions: {id: Math.random()}}}, true);
  },
  ...
  changeQuestion: function(poll, q, fields) {
    Polls.update({privateId: poll, 'questions.id': q}, {$set: {'questions.$.name': fields.name } });
  },
  ...
  selectVariant: function(poll, q, v, participant) {
    var update = {}, myId = Meteor.userId();
    ...
    update['questions.$.answers.'+myId] = v;
    update['dirty'] = true;
